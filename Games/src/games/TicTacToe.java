...
public class TicTacToe {
				...
				while (!inBounds) { // test to see if number is between 1-3
					...
					if (inBounds(position, bounds)) {
						if (!spaceTaken()) {
							...
							p("That space is taken!! Try again. ");
						}
	...
	// Description: Lets the players choose their pieces.
	...
	public boolean isWinner(char player) {
		...
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board.length; j++) {
				if (board[j][j] == player) {} else diag1Win = false;
				if (board[i][j] == player) {} else columnWin = false;
				if (board[j][i] == player) {} else rowWin = false;
			}
